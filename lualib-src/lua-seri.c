...
inline static struct block *blk_alloc(void) {
	struct block *b = (struct block*)zmalloc(sizeof(struct block));
	b->next = NULL;
	return b;
}
...
static inline void wb_integer(struct write_block *wb, int v, int type) {
	if (v == 0) {
...
static int _get_integer(lua_State *L, struct read_block *rb, int cookie) {
	switch (cookie) {
	case 0:
		return 0;
	case 1: {
...
static void _unpack_table(lua_State *L, struct read_block *rb, int array_size) {
	if (array_size == MAX_COOKIE-1) {
		uint8_t type = 0;
...
static void _push_value(lua_State *L, struct read_block *rb, int type, int cookie) {
	switch(type) {
	case TYPE_NIL:
		lua_pushnil(L);
		break;
...
static void _seri(lua_State *L, struct block *b) {
	uint32_t len = 0;
	memcpy(&len, b->buffer ,sizeof(len));

	len -= 4;
	uint8_t * buffer = (uint8_t*)zmalloc(len);
	uint8_t * ptr = buffer;
	int sz = len;
	if (len < BLOCK_SIZE - 4) {
